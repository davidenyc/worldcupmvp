...
export default function GroupsPage() {
  ...
  const content = groups.length ? (
        ...
        <div key={group.id} className="surface p-5">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <button
              ...
              className="rounded-full border border-line bg-surface-2 px-4 py-2 text-sm font-semibold text-deep"
            >
          ...
          <div className="mt-4">
            {group.creatorId === user.id && confirmDeleteId === group.id ? (
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => setConfirmDeleteId(null)} className="rounded-full border border-line px-3 py-1.5 text-sm font-semibold text-deep">
                  Cancel
                </button>
                <button
                  type="button"
    ...
    <EmptyState
      ...
      action={
        <Link href="/nyc/map" className="inline-flex rounded-full bg-gold px-5 py-2.5 text-sm font-bold text-[color:var(--fg-on-accent)]">
          Find a venue →
        </Link>
      }
    />
      ...
      <section className="surface max-w-xl p-5">
        ...
        <div className="mt-4 flex gap-2">
          ...
          <button
            ...
            className="rounded-2xl bg-gold px-5 py-3 text-sm font-bold text-[color:var(--fg-on-accent)]"
          >
            Join
          </button>
        </div>
