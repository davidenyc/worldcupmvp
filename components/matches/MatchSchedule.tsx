...
export function MatchSchedule({
  ...
  const filtered = useMemo(() => {
    return matches.filter((match) => {
      if (tab === "ny") return match.isNYNJ || match.stadiumName === "MetLife Stadium";
      if (tab === "group") return match.stage === "group";
      if (tab === "knockouts") return match.stage !== "group";
      return true;
    });
  }, [matches, tab]);
          ...
          <section key={date} className="surface p-5">
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {dayMatches.map((match) => {
                ...
                const local = match.isNYNJ || match.stadiumName === "MetLife Stadium";
                return (
