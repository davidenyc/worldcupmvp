...
export function MembershipPageClient({ initialFeature }: { initialFeature: string | null }) {
  const { tier } = useMembership();
  const [openFaq, setOpenFaq] = useState<number | null>(0);
  const [dismissedBanner, setDismissedBanner] = useState(false);

  const feature = initialFeature as PremiumFeature | null;
  const requiredTier = feature ? (FEATURE_GATES[feature].includes("fan") ? "fan" : "elite") : null;
  const currentTierMessage = useMemo(() => {
    if (tier === "fan") return "You're on Fan Pass - unlimited filters and saves.";
    if (tier === "elite") return "Supporter Elite - you have maximum access.";
    return "You're on the Free plan - 2 countries and 5 saves max.";
  }, [tier]);
